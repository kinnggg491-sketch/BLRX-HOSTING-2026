...
TELEGRAM_ADMIN_IDS = {
    x.strip() for x in os.getenv('TELEGRAM_ADMIN_IDS', '5176961308').split(',')
    if x.strip()
}
...
def telegram_api(method, params=None, timeout=15):
    """Small Telegram Bot API client without an extra Python package."""
...
def telegram_allowed(user_id):
    # If TELEGRAM_ADMIN_IDS is configured, only those users can use the log bot.
    # If it is empty, the bot stays disabled for safety.
    return str(user_id) in TELEGRAM_ADMIN_IDS


def telegram_send(chat_id, text):
    """Send a Telegram message, splitting long logs into Telegram-safe chunks."""
    if not TELEGRAM_BOT_TOKEN:
        return False
    text = str(text)
    if not text:
...
def get_log_text(server_id, max_lines=120):
    """Return the latest log lines for a server."""
    log_file = os.path.join(get_server_dir(server_id), 'output.log')
    if not os.path.exists(log_file):
        return 'لا توجد سجلات لهذا السيرفر.'
    try:
        with open(log_file, 'r', encoding='utf-8', errors='replace') as f:
            lines = f.readlines()
        if len(lines) > max_lines:
            lines = lines[-max_lines:]
        return ''.join(lines).strip() or 'لا توجد سجلات لهذا السيرفر.'
    except Exception as e:
        return f'تعذر قراءة السجلات: {e}'


def telegram_servers_text():
    users = load_users()
    rows = ['🖥️ BLRX HOSTING 2026', '']
    found = False
    for username, data in users.items():
        if username == 'admin':
            continue
        for server in data.get('servers', []) if isinstance(data.get('servers', []), list) else []:
            ...
            rows.append(
                ...
                f"📊 الحالة: {server.get('status', 'unknown')}\n"
                f"💻 CPU: {server.get('cpu_limit', 80)}%"
            )
            rows.append('')
    if not found:
        rows.append('لا توجد سيرفرات حالياً.')
    return '\n'.join(rows)
...
def telegram_handle_update(update):
    ...
    if not chat_id or not telegram_allowed(user_id):
        return
    ...
    if command in ('/start', '/help'):
        telegram_send(chat_id,
            '🤖 BLRX HOSTING 2026 - Log Bot\n\n'
            '/servers — عرض السيرفرات\n'
            '/logs <server_id> — عرض آخر السجلات\n'
            '/stats <server_id> — حالة وموارد السيرفر\n'
            '/clearlogs <server_id> — مسح السجلات')
        return

    if command == '/servers':
        telegram_send(chat_id, telegram_servers_text())
        return

    if command in ('/logs', '/log'):
        if not arg:
            telegram_send(chat_id, 'استخدم: /logs SERVER_ID')
            return
        server, owner = get_server_by_id(arg)
        if not server:
            telegram_send(chat_id, '❌ السيرفر غير موجود.')
            return
        body = get_log_text(arg)
        telegram_send(chat_id, f'📜 Logs | {arg} | {owner}\n\n{body}')
        return

    if command == '/stats':
        if not arg:
            telegram_send(chat_id, 'استخدم: /stats SERVER_ID')
            return
        server, owner = get_server_by_id(arg)
        if not server:
            telegram_send(chat_id, '❌ السيرفر غير موجود.')
            return
        stats = get_process_stats(server.get('pid')) if server.get('pid') else {'cpu_percent': 0, 'ram_display': '0 MB'}
        telegram_send(chat_id,
            f"📊 BLRX HOSTING 2026\n"
            f"🆔 {arg}\n"
            f"👤 {owner}\n"
            f"🟢 الحالة: {server.get('status', 'unknown')}\n"
            f"⚙️ CPU: {stats['cpu_percent']}% / حد {server.get('cpu_limit', 80)}%\n"
            f"🧠 RAM: {stats['ram_display']}\n"
            f"💾 Disk: {server.get('disk', '-')}\n"
            f"⏳ Expiry: {server.get('expiry', '-')}")
        return

    if command == '/clearlogs':
        if not arg:
            telegram_send(chat_id, 'استخدم: /clearlogs SERVER_ID')
            return
        log_file = os.path.join(get_server_dir(arg), 'output.log')
        try:
            if os.path.exists(log_file):
                os.remove(log_file)
            telegram_send(chat_id, f'✅ تم مسح سجلات السيرفر {arg}.')
        except Exception as e:
            telegram_send(chat_id, f'❌ فشل مسح السجلات: {e}')
        return

    telegram_send(chat_id, '❓ أمر غير معروف. أرسل /help')
    ...
    while True:
        try:
            result = telegram_api('getUpdates', {
                'offset': TELEGRAM_POLL_OFFSET,
                'timeout': 25,
                'allowed_updates': json.dumps(['message', 'edited_message'])
            }, timeout=35)
            if result and result.get('ok'):
                for update in result.get('result', []):
                    TELEGRAM_POLL_OFFSET = max(TELEGRAM_POLL_OFFSET, update.get('update_id', 0) + 1)
...
def start_telegram_log_bot():
    global TELEGRAM_BOT_STARTED
    if TELEGRAM_BOT_STARTED or not TELEGRAM_BOT_TOKEN or not TELEGRAM_ADMIN_IDS:
        return
    try:
        me = telegram_api('getMe', timeout=10)
        if not me or not me.get('ok'):
            print('⚠️ Telegram log bot: invalid token or Telegram unavailable')
            return
        TELEGRAM_BOT_STARTED = True
        print(f"✅ Telegram log bot connected: @{me['result'].get('username', 'bot')}")
        threading.Thread(target=telegram_poll_loop, daemon=True, name='telegram-log-bot').start()
    except Exception as e:
        print(f'⚠️ Telegram log bot error: {e}')
...
def create_default_files(server_dir):
    ...
    if not os.path.exists(main_py):
        with open(main_py, 'w', encoding='utf-8') as f:
            f.write('''# BLRX HOSTING 2026 - Default Bot
import time

print("=" * 40)
print("Bot is running on BLRX HOSTING 2026")
...
if __name__ == '__main__':
    start_telegram_log_bot()
    print("\n" + "=" * 50)
    print("🚀 BLRX HOSTING 2026 - FINAL")
    print("=" * 50)
