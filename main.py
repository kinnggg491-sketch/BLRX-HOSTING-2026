...
# Telegram Admin Bot
# ============================================
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "8709674266:AAFAWTqawJDEvEONGqOi5KleoLDDIxvULH4").strip()
TELEGRAM_ADMIN_IDS = {
    int(x.strip()) for x in os.getenv("TELEGRAM_ADMIN_IDS", "5176961308").split(",")
    if x.strip().lstrip("-").isdigit()
}
PUBLIC_BASE_URL = os.getenv("PUBLIC_BASE_URL", "https://blrx.up.railway.app").rstrip("/")
...
def tg_admin(update):
    try:
        return update.get("message", {}).get("from", {}).get("id") in TELEGRAM_ADMIN_IDS
    except Exception:
        return False
    ...
    for uname, data in users.items():
        ...
        for s in data.get("servers", []) if isinstance(data.get("servers", []), list) else []:
            if isinstance(s, dict):
                result.append((uname, s))
    return result

def tg_server(server_id):
    return get_server_by_id(server_id)

def tg_set_status(server_id, status, pid=None, stopped_by_user=False):
    users = load_users()
    for uname, data in users.items():
        if uname == "admin":
            continue
        for s in data.get("servers", []) if isinstance(data.get("servers", []), list) else []:
            if isinstance(s, dict) and s.get("server_id") == server_id:
                s["status"] = status
                s["pid"] = pid
                s["stopped_by_user"] = stopped_by_user
                if status == "running":
...
def tg_start_server(server_id):
    server, username = tg_server(server_id)
    if not server:
        return False, "السيرفر غير موجود."
    if server.get("status") == "running":
        return False, "السيرفر يعمل بالفعل."
    ...
    if not pid:
        return False, error or "فشل تشغيل السيرفر."
    tg_set_status(server_id, "running", pid, False)
    threading.Thread(target=monitor_bot, args=(server_id, pid), daemon=True).start()
    return True, f"تم تشغيل السيرفر. PID: {pid}"

def tg_stop_server(server_id):
    server, username = tg_server(server_id)
    if not server:
        return False, "السيرفر غير موجود."
    pid = server.get("pid")
    ...
    tg_set_status(server_id, "stopped", None, True)
    return True, "تم إيقاف السيرفر."

def tg_delete_server(server_id):
    server, username = tg_server(server_id)
    if not server:
        return False, "السيرفر غير موجود."
    if server.get("pid"):
    ...
    if username in users:
        ...
        users[username]["servers"] = [
            s for s in servers
            if not (isinstance(s, dict) and s.get("server_id") == server_id)
        ]
    ...
    try:
        ...
        pass
    return True, "تم حذف السيرفر وملفاته."

def tg_read_logs(server_id, limit=3500):
    ...
    try:
        with open(log_file, "r", encoding="utf-8", errors="replace") as f:
            data = f.read()
        return data[-limit:] if data else "لا توجد Logs."
    except Exception as e:
        return f"خطأ في قراءة Logs: {e}"

def tg_clear_logs(server_id):
    log_file = os.path.join(get_server_dir(server_id), "output.log")
    try:
...
def tg_create_server(args):
    if len(args) < 7:
        return ("الاستخدام:\n/create USERNAME PASSWORD RAM DISK CPU DAYS TYPE\n"
                "مثال:\n/create testuser Pass123 1GB 2GB 50 30 python")
    username, password, ram, disk = args[0], args[1], args[2], args[3]
    try:
        cpu, days = int(args[4]), int(args[5])
    except ValueError:
        return "CPU و DAYS يجب أن يكونا أرقامًا."
    server_type = args[6]
    if len(username) < 3 or len(password) < 4:
        return "اسم المستخدم أو كلمة المرور قصيرة."
    if not 10 <= cpu <= 100:
        return "CPU يجب أن يكون بين 10 و100."
    if not 1 <= days <= 365:
        return "مدة الاستضافة يجب أن تكون بين 1 و365 يومًا."
    ...
    full_link = f"{PUBLIC_BASE_URL}/{server_id}/login"
...
def telegram_poll_loop():
    if not TELEGRAM_BOT_TOKEN:
        print("Telegram Admin Bot: disabled (TELEGRAM_BOT_TOKEN is missing)")
        return
    offset = 0
    print("Telegram Admin Bot: enabled")
    while True:
        try:
            ...
            for update in updates:
                ...
                if command in ("/start", "/help"):
                    tg_send(chat_id,
                        "👑 BLRX HOSTING 2026 - Admin\n\n"
                        "/servers - عرض السيرفرات\n"
                        "/users - عرض المستخدمين\n"
                        "/create USER PASS RAM DISK CPU DAYS TYPE - إنشاء استضافة\n"
                        "/startserver ID - تشغيل\n"
                        "/stopserver ID - إيقاف\n"
                        "/restart ID - إعادة تشغيل\n"
                        "/delete ID - حذف\n"
                        "/stats ID - إحصائيات\n"
                        "/logs ID - Logs\n"
                        "/clearlogs ID - مسح Logs\n"
                        "/restartall - إعادة تشغيل الكل\n"
                        "/stopall - إيقاف الكل\n"
                        "/setcpu ID 80 - تغيير CPU\n"
                    )

                elif command == "/servers":
                    rows = tg_servers()
                    if not rows:
                        tg_send(chat_id, "📭 لا توجد سيرفرات.")
                    else:
                        out = ["🖥️ السيرفرات:"]
                        for uname, s in rows:
                            out.append(
                                f"• {s.get('server_id')} | {uname} | "
                                f"{s.get('status','stopped')} | CPU {s.get('cpu_limit',80)}%"
                            )
                        tg_send(chat_id, "\n".join(out))

                elif command == "/users":
                    users = load_users()
                    names = [u for u in users if u != "admin"]
                    tg_send(chat_id, "👥 المستخدمون:\n" + ("\n".join(f"• {u}" for u in names) if names else "لا يوجد."))

                elif command == "/create":
                    tg_send(chat_id, tg_create_server(parts[1:]))

                elif command in ("/startserver", "/stopserver", "/restart", "/delete", "/stats", "/logs", "/clearlogs"):
                    if len(parts) < 2:
                        tg_send(chat_id, f"الاستخدام: {command} SERVER_ID")
                        continue
                    sid = parts[1]

                    if command == "/startserver":
                        ok, reply = tg_start_server(sid)
                        tg_send(chat_id, ("✅ " if ok else "❌ ") + reply)

                    elif command == "/stopserver":
                        ok, reply = tg_stop_server(sid)
                        tg_send(chat_id, ("✅ " if ok else "❌ ") + reply)

                    elif command == "/restart":
                        ok1, reply1 = tg_stop_server(sid)
                        if not ok1:
                            tg_send(chat_id, "❌ " + reply1)
                            continue
                        time.sleep(2)
                        ok2, reply2 = tg_start_server(sid)
                        tg_send(chat_id, ("✅ " if ok2 else "❌ ") + reply2)

                    elif command == "/delete":
                        ok, reply = tg_delete_server(sid)
                        tg_send(chat_id, ("✅ " if ok else "❌ ") + reply)

                    elif command == "/stats":
                        server, username = tg_server(sid)
                        if not server:
                            tg_send(chat_id, "❌ السيرفر غير موجود.")
                            continue
                        if server.get("status") == "running" and server.get("pid"):
                            st = get_process_stats(server["pid"])
                            net_in, net_out = get_network_stats(server["pid"])
                            tg_send(chat_id,
                                f"📊 {sid}\n"
                                f"👤 {username}\n"
                                f"🟢 الحالة: running\n"
                                f"⚙️ CPU: {st['cpu_percent']}%\n"
                                f"🧠 RAM: {st['ram_display']}\n"
                                f"🌐 Read: {net_in}\n"
                                f"🌐 Write: {net_out}\n"
                                f"🎯 CPU Limit: {server.get('cpu_limit',80)}%"
                            )
                        else:
                            tg_send(chat_id,
                                f"📊 {sid}\n👤 {username}\n🔴 الحالة: stopped\n"
                        ...
                        server, _ = tg_server(sid)
                        if not server:
                            tg_send(chat_id, "❌ السيرفر غير موجود.")
                            continue
                        logs = tg_read_logs(sid)
                        tg_send(chat_id, f"📋 Logs — {sid}\n\n{logs}")
                        ...
                        if not server:
                            tg_send(chat_id, "❌ السيرفر غير موجود.")
                            continue
                        tg_send(chat_id, "✅ تم مسح Logs." if tg_clear_logs(sid) else "❌ فشل مسح Logs.")

                elif command == "/restartall":
                    count = 0
                    for _, s in tg_servers():
                        sid = s.get("server_id")
                        if sid and s.get("status") == "running":
                            tg_stop_server(sid)
                            time.sleep(1)
                            ok, _ = tg_start_server(sid)
                            count += 1 if ok else 0
                    tg_send(chat_id, f"🔄 تم إعادة تشغيل {count} سيرفر.")
                    ...
                    for _, s in tg_servers():
                        sid = s.get("server_id")
                        if sid and s.get("status") == "running":
                            ok, _ = tg_stop_server(sid)
                            count += 1 if ok else 0
                    tg_send(chat_id, f"🛑 تم إيقاف {count} سيرفر.")

                elif command == "/setcpu":
                    if len(parts) < 3:
                        tg_send(chat_id, "الاستخدام: /setcpu SERVER_ID 80")
                        continue
                    sid = parts[1]
                    try:
                        cpu = int(parts[2])
                    except ValueError:
                        tg_send(chat_id, "❌ قيمة CPU غير صحيحة.")
                        continue
                    if not 10 <= cpu <= 100:
                        tg_send(chat_id, "❌ CPU يجب أن يكون بين 10 و100.")
                        continue
                    server, _ = tg_server(sid)
                    if not server:
                        tg_send(chat_id, "❌ السيرفر غير موجود.")
                        continue
                    users = load_users()
                    _, username = tg_server(sid)
                    for s in users[username].get("servers", []):
                        if isinstance(s, dict) and s.get("server_id") == sid:
                            s["cpu_limit"] = cpu
                    save_users(users)
                    tg_send(chat_id, f"✅ تم تغيير CPU Limit إلى {cpu}%.")
